...
// carries the 404 boolean in the payload (error-state queries don't dehydrate), while
// fetchQuery primes the vue-query cache so the client hydrates without refetching.
const { data: isAffiliateLookupNotFound } = await useAsyncData(
  ...
  async () => {
    ...
    try {
      await queryClient.fetchQuery(getLikerInfoByIdQueryOptions({ likerId }))
      return false
    }
    catch (error) {
