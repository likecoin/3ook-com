...
async function computeForYouRecommendations(
  ...
  function getIsProductEligible(product: BookstoreCMSProduct): boolean {
    ...
    // Goods carry no genre, author or keywords, so they score on nothing and
    // would rank purely on the fallbacks. They are merchandised by tag instead.
    if (getIsGoodsProduct(product.productType)) return false
